...
::: callout-note
We use the filter to exclude everything else; only include those that belong to Shan state
:::
...
::: callout-tip
The printout above can be used to check your data. Alternatively, can use class to check the data object and ensure it is an sf data frame.
:::
...
::: callout-note
Using mutate to perform scaling on the data; use rename to make the field names shorter
:::
...
::: callout-tip
Instead of individual plots, we can stitch several together
:::
...
::: callout-tip
*K* argument specifies number of clusters. The more clusters there are, the more segregated it will be. Their position will not change; only their aggregation.
:::
...
::: callout-note
Use `as.factor` to convert numerical to a factor data type (like a character), use it in the form of ordinal/nominal scale for numerical data
:::
...
::: callout-important
The above is redundant, can go straight to below code due to latest version of spdep allowing *pl* argument to accept *sf* or *sfc*.
:::
...
::: callout-tip
Can use `plot(st_geometry(shan_sf))`, and then save the points using `st_coordinates(st_centroid(shan_sf))` . For future plots, if want to use `sf`, have to use `st_geometry`
:::
